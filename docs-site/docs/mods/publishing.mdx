...
Writing the mod itself is [Making mods](/docs/modding). This page is only about
publishing one.
...
directly — see [Sharing a mod](/docs/modding#sharing-a-mod) — just not how the
registry works.
...
- [Making mods](/docs/modding) — writing one in the first place.
- [Where the AI characters go](/docs/mods/ai-characters) — the population engine's
  spawn table.
